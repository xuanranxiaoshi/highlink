 ...
 * @Copyright：
 */
public class PrepareGantryFromKafka2 {
    ...
    private static void processGantryTrans(DataStream<GantryRawTransaction> gantryStream) {
        ...
        WatermarkStrategy<GantryCpcTransaction> watermarkStrategy = WatermarkStrategy
                .<GantryCpcTransaction>forBoundedOutOfOrderness(OutOfOrderGap)     // 乱序等待
                .withTimestampAssigner((rawTransaction, recordTimestamp) -> {
        ...
        // 3. 指定 watermark 策略，添加水位线
        SingleOutputStreamOperator<GantryCpcTransaction> gantryCpcTransWithWaterMark = gantryCpcStream.assignTimestampsAndWatermarks(watermarkStrategy);



        // 4. 根据 passId 对 cpc 数据流开窗
        SingleOutputStreamOperator<LinkedList<GantryCpcTransaction>> aggregateCpaStream = gantryCpcTransWithWaterMark
